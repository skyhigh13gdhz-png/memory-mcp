...
def _headers() -> dict[str, str]:
    if not GATEWAY_TOKEN: raise RuntimeError("MEMORY_GATEWAY_TOKEN is not configured")
...
async def memory_retain(content: str) -> dict[str, Any]:
    ...
    return await _gateway("/v1/memories/retain", {"content": content, "client_id": DEFAULT_CLIENT_ID})

@mcp.tool()
async def memory_recall(query: str, max_results: int = 10) -> dict[str, Any]:
    """快速查询过去的事实、决定、经历、项目进度和历史讨论。"""
    return await _gateway("/v1/memories/recall", {"query": query, "max_results": max(1, min(max_results, 100)), "client_id": DEFAULT_CLIENT_ID})
